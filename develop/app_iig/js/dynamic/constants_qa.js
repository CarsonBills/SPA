var JSConstants = {
    noMyItems: '<div class="empty-list">You have not added any items to your List yet.</div>',
    emptySearch: 'Please put in search query',
    creditsUrl: '/json/credits.json',
    loginUrl: '//books.wwnorton.com/books/ssl/WebLogin.aspx',
    invalidSiteCodeUrl: '//books.wwnorton.com/books/index.aspx',
    siteConfigUrl: '//qa2-services.wwnorton.com/getsiteconfig.php?',
    filterNavUrl: '//qa2-services.wwnorton.com/filter_nav.php',
    searchUrl: '//qa2-services.wwnorton.com/refinements.php',
    getDetailPageUrl: '//qa2-services.wwnorton.com/getpagedetail.php?',
    awsContentUrl: '//qa2-services.wwnorton.com/get_content.php?',
    saveTrackingUrl: '//qa2-services.wwnorton.com/tracking.php',
    likeUnlikeAssetUrl: '//qa2-services.wwnorton.com/like_unlike.php',
    getSavedFavsUrl: '//qa2-services.wwnorton.com/get_favorites.php',
    siteAssetsUrl: '',
    tabletLandspaceWidth: 1024,
    tabletPortaitWidth: 768,
    mobileWidth: 480,
    mobileWidthSM: 320,
    defaultPageSize: 10
};

module.exports = JSConstants;