var JSConstants = {
    noMyItems: '<div class="empty-list">You have not added any items to your List yet.</div>',
    emptySearch: 'Please put in search query',
    creditsUrl: '/json/credits.json',
    loginUrl: '//books.wwnorton.com/books/ssl/WebLogin.aspx',
    invalidSiteCodeUrl: '//books.wwnorton.com/books/index.aspx',
    siteConfigUrl: '//dev2-services.wwnorton.com/getsiteconfig.php?',
    searchUrl: '//dev2-services.wwnorton.com/refinements.php',
    filterNavUrl: '//dev2-services.wwnorton.com/filter_nav.php',
    getDetailPageUrl: '//dev2-services.wwnorton.com/getpagedetail.php?',
    awsContentUrl: '//dev2-services.wwnorton.com/get_content.php?',
    saveTrackingUrl: '//dev2-services.wwnorton.com/tracking.php',
    likeUnlikeAssetUrl: '//dev2-services.wwnorton.com/like_unlike.php',
    getSavedFavsUrl: '//dev2-services.wwnorton.com/get_favorites.php',
    siteAssetsUrl: '',
    tabletLandspaceWidth: 1024,
    tabletPortaitWidth: 768,
    mobileWidth: 480,
    mobileWidthSM: 320,
    defaultPageSize: 10
};

module.exports = JSConstants;